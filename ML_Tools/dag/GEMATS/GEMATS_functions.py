...
def open_meteo_forecast():
    for idx, id in enumerate(df_marketarea['id']):
        ...
        params = {
            "latitude": latitude,
            "longitude": longitude,
            "hourly": "temperature_2m,rain,windspeed_10m,shortwave_radiation,relative_humidity_2m,pressure_msl",
            "forecast_days": 7,
            "timezone": "auto"
        }
        response = requests.get(url, params=params)
        ...
        df['SpecificHumidity'] = compute_specific_humidity(df['temperature_2m'], df['pressure_msl'], df['relative_humidity_2m'])
        for idx1, date in enumerate(df['time']):
            ...
            db_jj = {"area":id,"start_time":start_time,
                            ...
                            "day":day,"year":year,"month":month,"weekday":weekday,"velocity":float(velocity)}
