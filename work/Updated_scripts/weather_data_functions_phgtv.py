...
def get_pair(plant_height):
    lst = [50,80,100,120,150]
    for i in range(len(lst)-1):
        if lst[i] <= plant_height < lst[i+1]:
            return [lst[i], lst[i+1]]
    return [lst[-2], lst[-1]] if plant_height >= lst[-1] else [lst[0], lst[1]]

def wind_shear(u1,u2,h1,h2,h_targ):
    alpha = np.log(u2 / u1) / np.log(h2 / h1)
    return u1 * (h_targ / h1) ** alpha

def velocity_plnt_hgt(nearest_rows,plant_height,vel):
    h1,h2 = get_pair(plant_height)
    df_h1_h2 = nearest_rows[nearest_rows['lev'].isin([h1, h2])]
    df_pivot = df_h1_h2.pivot_table(index=['time', 'lon', 'lat'], 
                                    columns='lev', values=vel).reset_index()
    df_pivot.columns.name = None
    df_pivot = df_pivot.rename(columns={h1: 'u1', h2: 'u2'})
    def wind_shear(u1, u2):
        alpha = np.log(u2 / u1) / np.log(h2 / h1)
        return u1 * (plant_height / h1) ** alpha
    df_pivot[vel+'_plnt_hgt'] = wind_shear(df_pivot['u1'], df_pivot['u2'])
    df_pivot = df_pivot[vel+'_plnt_hgt']
    return df_pivot
                    ...
                    for idx, plant_name in enumerate(df_plant["Farm"]):
                        ...
                        for file_name in data_dict.keys():
                            ...
                            if "u_wind" in file_name:
                                df_u_plant_hgt = velocity_plnt_hgt(nearest_rows,plant_height,'u')
                                df1 = pd.DataFrame()
                                ...
                                nearest_rows = pd.concat([nearest_rows.reset_index(drop=True),df_u_plant_hgt.reset_index(drop=True)],axis=1)
                            if "v_wind" in file_name:
                                df_v_plant_hgt = velocity_plnt_hgt(nearest_rows,plant_height,'v')
                                df2 = pd.DataFrame()
                                for idx,lev in enumerate(levels):
                                    if idx==0:
                                ...
                                nearest_rows = pd.concat([nearest_rows.reset_index(drop=True),df_v_plant_hgt.reset_index(drop=True)],axis=1)
                        ...
                        plant_data['WindSpeed_'+str(plant_height)+'m(m/s)'] = np.sqrt(plant_data['u_plnt_hgt']**2 + plant_data['v_plnt_hgt']**2).round(2)
                        plant_data['WindDirection_'+str(plant_height)+'m(deg)'] = (np.degrees(np.arctan2(plant_data['u_plnt_hgt'],plant_data['v_plnt_hgt'])) + 180) % 360
                        plant_data['WindDirection_'+str(plant_height)+'m(deg)'] = plant_data['WindDirection_'+str(plant_height)+'m(deg)'].round(2)
                        plant_data = plant_data.drop(columns=['u_plnt_hgt','v_plnt_hgt'])
                        for lev in levels:
                            plant_data['WindSpeed_'+str(lev)+'m(m/s)'] = np.sqrt(plant_data['u_'+str(lev)]**2 + plant_data['v_'+str(lev)]**2).round(2)
                            plant_data['WindDirection_'+str(lev)+'m(deg)'] = (np.degrees(np.arctan2(plant_data['u_'+str(lev)],plant_data['v_'+str(lev)])) + 180) % 360
                            plant_data['WindDirection_'+str(lev)+'m(deg)'] = plant_data['WindDirection_'+str(lev)+'m(deg)'].round(2)
                        ...
                        for date in dates_list:
                            ...
                            cols_to_round = df_date_5mins.columns.difference(['lat', 'lon'])
                            df_date_5mins[cols_to_round] = df_date_5mins[cols_to_round].round(2)
